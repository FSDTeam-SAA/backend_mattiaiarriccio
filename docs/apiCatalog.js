  ...
  groups: [
    ...
    },
    {
      ...
      routes: [
        ...
        { method: 'GET', path: '/chat/config', description: 'Welcome message plus three independent welcome-screen prompt lists for the caller language: ordinary `suggestedQuestions[]`, `liveInfoSuggestions[]` buttons, and `webSearchExamples[]`; each entry is `{ id, title, prompt, icon, order, requiresLocation }`. `webSearchSection` contains the localized title/description. Also returns `webSearchEnabled` (false when the feature is off or no source is approved, in which case both Web Search lists are empty).' },
        { method: 'POST', path: '/chat/messages', description: `Send a message, generate the reply, store it, and return both messages. When provided, \`emergencyType\` is used directly; otherwise the conversation type is reused or it falls back to \`${DEFAULT_AI_EMERGENCY_TYPE}\`. Optional \`location: { city, region, country, timezone, latitude, longitude, accuracyMeters }\` uses the measured point for weather while only the coarse fields bias Web Search and are remembered. \`forceWebSearch: true\` is reserved for a dedicated Live Information action; typed chat stays trigger-driven. Response adds \`usedWebSearch\`, \`sources[]\`, and \`liveInfoLimited\`.` },
        { method: 'POST', path: '/chat/messages/stream', description: 'Streaming form of `/chat/messages`, with the same location and `forceWebSearch` fields. Emits Server-Sent Events: meta, status, weather, delta, done, or error. `status` carries `{ state: "searching" }` while live sources are being consulted.' },
    ...
    },
    {
      ...
      routes: [
        ...
        { method: 'GET', path: '/admin/live-info-suggestions', description: 'List the chat welcome-screen prompts. Filter by `kind` (`live_info` | `suggested_question`), `language`, and `active`.' },
        { method: 'POST', path: '/admin/live-info-suggestions', description: 'Create a welcome-screen prompt (icon, title, prompt, kind, requiresLocation, language, order). A prompt may contain `{location}`, which the app fills with the place the user chooses.' },
        { method: 'PATCH', path: '/admin/live-info-suggestions/:suggestionId', description: 'Edit, reorder, or enable/disable a welcome-screen prompt.' },
        { method: 'DELETE', path: '/admin/live-info-suggestions/:suggestionId', description: 'Remove a welcome-screen prompt.' },
