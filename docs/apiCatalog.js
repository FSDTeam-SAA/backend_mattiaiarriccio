  ...
  groups: [
    ...
    },
    {
      ...
      routes: [
        ...
        { method: 'GET', path: '/chat/config', description: 'Welcome message plus the two welcome-screen prompt lists for the caller language: `suggestedQuestions[]` and `liveInfoSuggestions[]`, each entry `{ id, title, prompt, icon, order, requiresLocation }`. Also `webSearchEnabled` (false when the feature is off or no source is approved, in which case `liveInfoSuggestions` is empty).' },
